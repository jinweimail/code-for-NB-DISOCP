...
for iNmc=1:Nmc
    inter1=-45:-35;
    inter2=45:55;
    theta_i1=inter1(ceil(rand*length(inter1)));
    theta_i2=inter1(ceil(rand*length(inter2)));
    theta_i_move=[theta_i1 theta_i2];
    a_i_move=exp(-1j*(0:M-1)'*2*pi*dl*sin(theta_i_move*derad));
    ...
    for insnap=1:length(nsnapshot) 
        ...
        SINR_SMI(iNmc,insnap)=Ps*(abs(w_SMI'*a_s_r))^2/(w_SMI'*Rin_move*w_SMI);
        
        w_EIG=eigendec(RE,ninter,a_s_n);
        SINR_EIG(iNmc,insnap)=Ps*(abs(w_EIG'*a_s_r))^2/(w_EIG'*Rin_move*w_EIG);
    
        [a_RCB lambda_RCB] = RCBsphere(RE,a_s_n,epsilon_RCB);
        w_RCB=inv(RE)*a_RCB/(a_RCB'*inv(RE)*a_RCB);
        SINR_RCB(iNmc,insnap)=Ps*(abs(w_RCB'*a_s_r))^2/(w_RCB'*Rin_move*w_RCB);
        
        [R_CMT]=CMT(M,delta_CMT,RE);
        w_CMT=inv(R_CMT)*a_s_n/(a_s_n'*inv(R_CMT)*a_s_n);
        SINR_CMT(iNmc,insnap)=Ps*(abs(w_CMT'*a_s_r))^2/(w_CMT'*Rin_move*w_CMT);
     
        [R_PDL,w_PDL]=PDL_Mao(M,InterRange1,InterRange2,nullwidth,K_PDL,RE,lambda_PDL,a_s_n,SenLoc);
        SINR_PDL(iNmc,insnap)=Ps*(abs(w_PDL'*a_s_r))^2/(w_PDL'*Rin_move*w_PDL);
        
        w_LCSS=LCSS(RE,M,InterRange1,InterRange2,nullwidth,eta_LCSS,a_s_n,SenLoc);
        SINR_LCSS(iNmc,insnap)=Ps*(abs(w_LCSS'*a_s_r))^2/(w_LCSS'*Rin_move*w_LCSS);
        
        w_NB_DISOCP_WC = NB_DISOCP_WC_LA(RE,BeamRange,SenLoc,Ripple,StepSize,epsilon,InterRange1,InterRange2,eta_NB_DISOCP);
        SINR_NB_DISOCP_WC(iNmc,insnap)=Ps*(abs(w_NB_DISOCP_WC(:,end)'*a_s_r))^2/(w_NB_DISOCP_WC(:,end)'*Rin_move*w_NB_DISOCP_WC(:,end));
    end
end
