...
Nmc=2;
for iNmc=1:Nmc
    for ipoint=1:length(pointerror)
        theta_s_n=theta_s_r+pointerror(ipoint);
        a_s_n=exp(-1j*(0:M-1)'*2*pi*dl*sin(theta_s_n*derad));
        ...
        SINR_OPT(iNmc,ipoint)=Ps*a_s_r'*inv(Rin)*a_s_r;
    
        w_SMI=inv(RE)*a_s_n/(a_s_n'*inv(RE)*a_s_n);
        SINR_SMI(iNmc,ipoint)=Ps*(abs(w_SMI'*a_s_r))^2/(w_SMI'*Rin*w_SMI);
        
        w_EIG=eigendec(RE,ninter,a_s_n);
        SINR_EIG(iNmc,ipoint)=Ps*(abs(w_EIG'*a_s_r))^2/(w_EIG'*Rin*w_EIG);
        
        [a_RCB lambda_RCB] = RCBsphere(RE,a_s_n,epsilon_RCB);
        w_RCB=inv(RE)*a_RCB/(a_RCB'*inv(RE)*a_RCB);
        SINR_RCB(iNmc,ipoint)=Ps*(abs(w_RCB'*a_s_r))^2/(w_RCB'*Rin*w_RCB);
        
        [R_CMT]=CMT(M,delta_CMT,RE);
        w_CMT=inv(R_CMT)*a_s_n/(a_s_n'*inv(R_CMT)*a_s_n);
        SINR_CMT(iNmc,ipoint)=Ps*(abs(w_CMT'*a_s_r))^2/(w_CMT'*Rin*w_CMT);
       
        [R_PDL,w_PDL]=PDL_Mao(M,InterRange1,InterRange2,nullwidth,K_PDL,RE,lambda_PDL,a_s_n,SenLoc);
        SINR_PDL(iNmc,ipoint)=Ps*(abs(w_PDL'*a_s_r))^2/(w_PDL'*Rin*w_PDL);
        
        w_LCSS=LCSS(RE,M,InterRange1,InterRange2,nullwidth,eta_LCSS,a_s_n,SenLoc);
        SINR_LCSS(iNmc,ipoint)=Ps*(abs(w_LCSS'*a_s_r))^2/(w_LCSS'*Rin*w_LCSS);
       
        w_NB_DISOCP_WC = NB_DISOCP_WC_LA(RE,BeamRange,SenLoc,Ripple,StepSize,epsilon,InterRange1,InterRange2,eta_NB_DISOCP);
        SINR_NB_DISOCP_WC(iNmc,ipoint)=Ps*(abs(w_NB_DISOCP_WC(:,end)'*a_s_r))^2/(w_NB_DISOCP_WC(:,end)'*Rin*w_NB_DISOCP_WC(:,end));
end
end

figure(1);
plot(pointerror,10*log10(sum(SINR_OPT)/Nmc),'k-','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_SMI)/Nmc),'+--','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_EIG)/Nmc),'x--','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_RCB)/Nmc),'p--','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_CMT)/Nmc),'md-','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_PDL)/Nmc),'gs-','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_LCSS)/Nmc),'b^-','LineWidth',1);hold on
plot(pointerror,10*log10(sum(SINR_NB_DISOCP_WC)/Nmc),'ro-','LineWidth',1);    
legend('OPTIMAL SINR','SMI','EIG','RCB','CMT','PDL','LCSS','NB-DISOCP');
% xlabel('干扰角度偏移（度）');
% ylabel('增益(dB)');
xlabel('Point error（degree）');
